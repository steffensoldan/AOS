...
- **Secrets & Credentials (Zero-Plaintext-Policy):** Passwörter, Tokens, SSH-Keys oder API-Keys dürfen NIEMALS im Quellcode oder in Skripten stehen. Bei jeder Bearbeitung bestehender Skripte müssen vorhandene Hardcodierungen unverzüglich auf Umgebungsvariablen (`os.getenv()`) umgestellt und `.env.example` als Vorlage im Repo gepflegt werden.
